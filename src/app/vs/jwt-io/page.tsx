  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans">
      ...
      <main className="max-w-3xl mx-auto px-4 py-16">
        ...
        <p className="text-lg text-zinc-400 mb-8">SafeJSON decodes JWT headers and claims in a browser-local workflow. jwt.io is the most popular online JWT debugger, but it processes tokens on its server. For production tokens, this matters.</p>
        <div className="overflow-x-auto mb-12">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {[["JWT processing","Browser-local header and claim decoding","Server-side. Token transmitted to jwt.io servers."],["Production token policy","Do not paste production secrets unless your policy allows local inspection.","No — production tokens should not be sent to third-party servers."],["Verifiable privacy","Yes — open DevTools Network tab.","No — token is sent via network request."],["Open source","Yes (MIT)","Yes (jwt.io debugger is open source)"],["Extra JSON tools","Diff, Validator, JSONPath, Schema, Formatter, CSV conversion","None — JWT decoder only"],["Price","Free / $5 Pro","Free"]].map(([f,s,j],i)=><tr key={i} className="hover:bg-white/[0.02]"><td className="py-3 pr-4 text-zinc-300 font-medium">{f}</td><td className="py-3 pr-4 text-zinc-400">{s}</td><td className="py-3 text-zinc-500">{j}</td></tr>)}
            </tbody>
          </table>
        </div>
        <p className="text-sm text-zinc-500 mb-4">jwt.io is a great learning and debugging tool. But if you are working with production authentication tokens or security-sensitive JWTs, use a workflow that matches your security policy and can be verified in DevTools.</p>
