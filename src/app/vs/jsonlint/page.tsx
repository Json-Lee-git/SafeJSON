...
export const metadata: Metadata = {
  ...
  description: "SafeJSON validates pasted JSON in a browser-local workflow with no pasted-content upload. JSONLint processes on servers. Compare features, privacy, and security.",
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans">
      ...
      <main className="max-w-3xl mx-auto px-4 py-16">
        ...
        <p className="text-lg text-zinc-400 mb-8">SafeJSON validates pasted JSON in a browser-local workflow. JSONLint processes on its server — your data makes a round trip. Here is the comparison.</p>
        <div className="overflow-x-auto mb-12">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {[["Processing","Browser-local workflow for pasted input","Server-side, data transmitted for validation"],["Privacy verifiable","Yes - open DevTools and check no request contains pasted JSON","No — data leaves your browser"],["Error reporting","Line and column numbers","Error message only"],["Open source","Yes (MIT)","No"],["Ads","None","Yes"],["Extra tools","Diff, JWT, JSONPath, Schema, CSV conversion","None"],["Price","Free / $5 Pro","Free (ad-supported)"]].map(([f,s,j],i)=><tr key={i} className="hover:bg-white/[0.02]"><td className="py-3 pr-4 text-zinc-300 font-medium">{f}</td><td className="py-3 pr-4 text-zinc-400">{s}</td><td className="py-3 text-zinc-500">{j}</td></tr>)}
            </tbody>
          </table>
        </div>
        <p className="text-sm text-zinc-500 mb-4">JSONLint is a useful tool, but it processes JSON server-side. If you paste sensitive data — API keys, tokens, customer PII — it leaves your browser. SafeJSON keeps validation browser-local for pasted input.</p>
